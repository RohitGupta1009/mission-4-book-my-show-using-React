...
const MovieInfo = () => {
  ...
  const genres=movie.genres?.map(({name})=>name).join(", "); //This techniques is known as optional chaining and (", ") will remove the array bar put a " then one element then , and space then 2nd element till last
  const rentMovies = () => {
    setIsOpen(true);
    setPrice(149);
  };
